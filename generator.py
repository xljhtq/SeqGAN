...
class Generator(object):
    def __init__(self,
        ...
        with tf.variable_scope('generator'):  ## use RNN
            wordInitial = tf.constant(word_vocab.word_vecs)
            self.g_embeddings = tf.get_variable("word_embedding",
                                                trainable=False,
        ...
        self.x = tf.placeholder(tf.int32, shape=[self.batch_size,
                                                 self.sequence_length])  # sequence of  true tokens
        ...
        with tf.device("/cpu:0"):
            self.processed_x = tf.transpose(tf.nn.embedding_lookup(self.g_embeddings, self.x),
                                            perm=[1, 0, 2])  # 第0维和第1维进行转换: seq_length * batch_size * emb_dim

        # todo:   ##############1. while loop for generating tokens by time step ##############
        self.h0 = tf.zeros([self.batch_size, self.hidden_dim])  # 64*50
        self.h0 = tf.stack([self.h0, self.h0])
        gen_o = tensor_array_ops.TensorArray(dtype=tf.float32,
                                             size=self.sequence_length,
                                             dynamic_size=False,
                                             infer_shape=True)  # build dynamic array, 动态存放内容到数组里
        ...
        def _g_recurrence(i, x_t, h_tm1, gen_o, gen_x):
            ...
            o_t = self.g_output_unit(h_t)  # batch * vocab
            log_prob = tf.log(tf.nn.softmax(o_t))
            next_token = tf.cast(tf.reshape(tf.multinomial(log_prob, 1), [self.batch_size]),  # 根据logits中每个类别的概率采样1个
                                 tf.int32)  # 获得0-5000 中的index, batch_size个
            x_tp1 = tf.nn.embedding_lookup(self.g_embeddings, next_token)  # batch * emb_dim
            gen_o = gen_o.write(i, tf.reduce_sum(tf.multiply(tf.one_hot(next_token, self.num_emb, 1.0, 0.0),
                                                             tf.nn.softmax(o_t)),
                                                 1))  # save prob of next_token, batch * 1
            gen_x = gen_x.write(i, next_token)  # save indices, batch_size * 1
        ...
        # todo:  ##############2. supervised pretraining for generator ##############
        ...
        ta_emb_x = tensor_array_ops.TensorArray(dtype=tf.float32,
                                                size=self.sequence_length)
        ta_emb_x = ta_emb_x.unstack(self.processed_x)  # seq_length * batch_size * emb_dim 拆分成多个真实的seq

        def _pretrain_recurrence(i, x_t, h_tm1, g_predictions):
            h_t = self.g_recurrent_unit(x_t, h_tm1)
            o_t = self.g_output_unit(h_t)
            g_predictions = g_predictions.write(i, tf.nn.softmax(o_t))  # 输出 batch * vocab_size的概率
            x_tp1 = ta_emb_x.read(i)  # todo: 读取真实seq的第i个作为x输入
        ...
        #####2.1 pretraining loss: based on MLE 的 loss (3维转2维进行计算) ###
        ...
        #####2.2 training updates  ###梯度裁剪使得效果好
        ...
        self.g_loss = -tf.reduce_sum(
            tf.reduce_sum(
                tf.one_hot(tf.to_int32(tf.reshape(self.x, [-1])), self.num_emb, 1.0, 0.0) *
                tf.log(tf.clip_by_value(tf.reshape(self.g_predictions, [-1, self.num_emb]), 1e-20, 1.0)), axis=1) *
            tf.reshape(self.rewards, [-1]))

        g_opt = self.g_optimizer(self.learning_rate)
        self.g_grad, _ = tf.clip_by_global_norm(tf.gradients(self.g_loss, self.g_params),
                                                self.grad_clip)
    ...
    def create_recurrent_unit(self, params):
        ...
        def unit(x, hidden_memory_tm1):
            previous_hidden_state, c_prev = tf.unstack(hidden_memory_tm1)  ## 64*50 , 64*50
