...
class ROLLOUT(object):
    def __init__(self, lstm, update_rate):
        ...
        self.g_recurrent_unit = self.create_recurrent_unit()  # 复用generator的参数
        self.g_output_unit = self.create_output_unit()  # 复用generator的参数
        ...
        ta_emb_x = ta_emb_x.unstack(self.processed_x)  ## 其中一个list是 batch_size*emb_dim

        ta_x = tensor_array_ops.TensorArray(dtype=tf.int32, size=self.sequence_length)
        ta_x = ta_x.unstack(tf.transpose(self.x, perm=[1, 0]))  ## 其中一个list是 batch_size*1
        #####################################################################################################
    ...
    def get_reward(self, sess, input_x, rollout_num, discriminator):
        rewards = []
        for i in range(rollout_num):  ## 做rolllout_num次，再取平均. 例如：  given "I", 做rolllout_num次 “I” 的MC操作
            for given_num in range(1, self.sequence_length):
                ...
                if i == 0:
                    ...
                    rewards[given_num - 1] += ypred  ## 对于第given_num的rewards 进行求和，之后会平均
    ...
    def update_recurrent_unit(self):
        # self.Wi 表示原来的， self.lstm.Wi表示更新后的权重
